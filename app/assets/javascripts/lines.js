...
function Nighter(){
  ...
  var styleTag, currStyle, isNight;
  var that = this;
  this.apply = function(){
    if(that.isNight){
      this.mapSettings = {
  ...
  this.init = function(){
    var now = new Date();
    that.isNight = ((now.getHours() >= 20 && now.getMinutes >= 38) || now.getHours() > 21) || ((now.getHours() <= 6 && now.getMinutes() < 25) || now.getHours() < 6);
    that.apply();
  ...
  this.swap = function(){
    that.isNight = !that.isNight;
    that.apply();
    $('link#styles').attr('href', currStyles);
  }
  return this;
}

var nighter = new Nighter();
...
var sketchProc = (function($p) {
    ...
    var yDir = 1;
    function draw() {
        $p.background(nighter.mapSettings.white);

        for (var i = 1; i < $p.width_base; i++) {
            my_line_3(coord_x, coord_y, ($p.width_base - i) * $p.width_base - ($p.width_base / 2), i % 2 == 0 ? nighter.mapSettings.white : nighter.mapSettings.black);
        }
        var i = 1;
